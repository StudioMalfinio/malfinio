...
# Create required labels for GitHub
create_github_labels() {
    local epic_name="$1"
    
    # Create standard labels (ignore if they already exist)
    gh label create "epic" --description "Epic tracking label" --color "8B5CF6" 2>/dev/null || true
    gh label create "task" --description "Development task" --color "10B981" 2>/dev/null || true
    gh label create "epic:$epic_name" --description "Epic: $epic_name" --color "8B5CF6" 2>/dev/null || true
    
    # Create phase labels
    for phase in {1..7}; do
        gh label create "phase:$phase" --description "Phase $phase tasks" --color "F59E0B" 2>/dev/null || true
    done
}
...
sync_epic() {
    ...
    if [[ "$git_platform" == "github" ]]; then
        ...
        create_github_labels "$epic_name"
    fi
    ...
    if [[ "$git_platform" == "gitlab" ]]; then
        ...
        # Get repo
        repo=$(git remote get-url origin | sed 's/.*github.com[:/]*//; s/\.git$//' 2>/dev/null || echo "unknown/repo")
    fi
...
EOF
    ...
    if [[ "$git_platform" == "gitlab" ]]; then
        ...
        local epic_output=$(gh issue create \
            --title "Epic: $epic_name" \
            --body-file /tmp/epic-body.md \
            --label "epic,epic:$epic_name" 2>&1 || echo "Error creating epic")
        
        if [[ "$epic_output" == *"Error"* ]]; then
            echo "❌ Failed to create epic: $epic_output"
            exit 1
        fi
        
        epic_url="$epic_output"
        epic_number=$(echo "$epic_output" | grep -o '[0-9]*$')
        
        if [[ -z "$epic_number" ]]; then
            echo "❌ Could not extract epic number from: $epic_output"
            exit 1
        fi
    fi
